...
## Implementação — setup e execução

Decisões técnicas detalhadas: [`ARCHITECTURE.md`](./ARCHITECTURE.md).

### Pré-requisitos

- [Bun](https://bun.sh) 1.x
- Docker + Docker Compose (PostgreSQL 16 + MiniStack)
...
| GET | `/wallets/:walletId/ledger` | Ledger paginado (cursor opaco) |
| POST | `/wallets/:walletId/reconciliation` | Reconciliar saldo vs ledger |
| POST | `/wagering/transactions` | Submeter transação (header `Idempotency-Key` obrigatório) |
| GET | `/wagering/transactions/:transactionId` | Consultar por ID interno |
| GET | `/providers/:providerId/wagering/transactions/:externalTransactionId` | Consultar por ID externo |
| GET | `/health/live` | Liveness |
| GET | `/health/ready` | Readiness (PostgreSQL + SQS) |
...
RUN_INTEGRATION_TESTS=true RUN_MESSAGING_TESTS=true bun run test:messaging
RUN_INTEGRATION_TESTS=true bun run test:concurrency
```

Os testes de integração, messaging e concorrência usam PostgreSQL (`wagering/wagering@localhost:5432`) e MiniStack (`localhost:4566`) reais.

### Stack

Bun, NestJS, MikroORM 6, PostgreSQL 16, MiniStack (SQS), `@aws-sdk/client-sqs`, `prom-client`, `decimal.js`.
